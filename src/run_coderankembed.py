...
def main():
    ...
    parser.add_argument("--model", type=str, default=os.environ.get("CODERANK_MODEL", r"/home/jiaxin/CodeRankEmbed"))
    ...
    outputs_dir = Path(cfg["outputs_dir"]) / "coderankembed"
    bins_dir = Path(cfg.get("bins_dir", "bin"))
    logs_dir = Path(cfg["logs_dir"]) / "coderankembed"
    ...
    logging.info(f"Load pretrained SentenceTransformer: {args.model}")
    try:
        encoder = SentenceTransformer(args.model, device=device, trust_remote_code=True)
        try:
            encoder.max_seq_length = int(args.max_seq_len)
        except Exception:
            pass
    ...
    for (repo, commit), items in groups.items():
        ...
        try:
            try:
                ...
                approx_mb = (len(texts) * max(dim, 1) * 4) / (1024 * 1024)
                logging.info(f"embedding_dim={dim} docs={len(texts)} approx_cpu_mem_for_doc_embs_mb={approx_mb:.1f}")
            except Exception:
                pass
            ...
            if device == "cuda" and is_oom:
                ok = False
                try:
                    logging.info("retry encode with batch_size=1")
                    t0 = time.time()
                    doc_embs = encoder.encode(texts, batch_size=1, convert_to_numpy=True, normalize_embeddings=True, show_progress_bar=True)
                    logging.info(f"encoded {len(texts)} docs in {time.time() - t0:.3f}s for {dname} after retry batch_size=1")
                    ok = True
                except Exception as e2:
                    logging.warning(f"retry encode with batch_size=1 failed: {e2}")
                if not ok:
                    try:
                        logging.info("reload encoder on cpu and retry encode")
                        encoder = SentenceTransformer(args.model, device="cpu", trust_remote_code=True)
                        try:
                            encoder.max_seq_length = int(args.max_seq_len)
                        except Exception:
                            pass
                        ...
                        doc_embs = encoder.encode(texts, batch_size=1, convert_to_numpy=True, normalize_embeddings=True, show_progress_bar=True)
                        device = "cpu"
                        logging.info(f"encoded {len(texts)} docs in {time.time() - t0:.3f}s for {dname} on cpu")
                        ok = True
                    except Exception as e3:
                        logging.exception(f"encode failed on cpu for {dname}")
                if not ok:
                    for it in items:
                        save_output(bins_dir / it["split"], it, [], 0, 0.0, {"error": "encode_failed", "exception": msg})
                    continue
            else:
        ...
        for it in items:
            ...
            try:
                ...
                if device == "cuda" and is_oom:
                    try:
                        logging.info("reload encoder on cpu and retry query encode")
                        encoder = SentenceTransformer(args.model, device="cpu", trust_remote_code=True)
                        try:
                            encoder.max_seq_length = int(args.max_seq_len)
                        except Exception:
                        ...
                        logging.exception(f"query encode failed on cpu for {it['instance_id']}")
                        for it2 in [it]:
                            save_output(bins_dir / it2["split"], it2, [], total, 0.0, {"error": "query_encode_failed", "exception": msg})
                        continue
                else:
