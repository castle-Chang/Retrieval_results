...
def main():
    ...
    parser.add_argument("--model", type=str, default=os.environ.get("MGTE_MODEL",  r"/home/jiaxin/gte-multilingual-base"))
    ...
    if dev == "cpu":
        ...
        idx = dev.split(":", 1)[1] if ":" in dev else "2"
    ...
    outputs_dir = Path(cfg["outputs_dir"]) / "mgte"
    bins_dir = Path(cfg.get("bins_dir", "bin"))
    logs_dir = Path(cfg["logs_dir"]) / "mgte"
    ...
    try:
        ...
        logging.warning("model load failed for provided path")
        model = None
    for (repo, commit), items in groups.items():
        ...
        if model is None:
            ...
            for it in items:
                save_output(bins_dir / it["split"], it, [], 0, 0.0, {"error": "model_load_failed"})
            continue
        ...
        try:
            ...
            t0 = time.time()
            doc_embs = model.encode(texts, batch_size=args.batch_size, convert_to_numpy=True, normalize_embeddings=True, show_progress_bar=True)
        ...
        for it in items:
            ...
            q = f"query: {it['query'] or ''}"
            try:
                q_emb = model.encode([q], convert_to_numpy=True, normalize_embeddings=True, show_progress_bar=True)[0]
            except Exception as e:
